	...
	methods: {
		...
		calcMax(n) {
			...
			if (n > 8 && n < 13) {
				...
				curLoad += 7.9 + ((n - 13)*1.6)
			} else if (n >= 21 && n < 26) {
				curLoad += 20.6 + ((n - 21)*1.6)
			} //continue until 99
...
<template>
	...
	<div class="content">
		...
		<div v-if="isCalculated==true && isAdvanced==false" class="results">
			...
			<p v-else-if="curLoad<=roundedTo(maxLoad*.299, 1)">
				{{ status }} LIGHT
			</p>
			<p v-else-if="curLoad>roundedTo(maxLoad*.299, 1) && curLoad<=roundedTo(maxLoad*.699, 1)">
				{{ status }} MEDIUM
			</p>
			<p v-else-if="curLoad>roundedTo(maxLoad*.699, 1) && curLoad<=roundedTo(maxLoad*.999, 1)">
				{{ status }} HEAVY
			</p>
		...
		<div v-if="isCalculated==true && isAdvanced==true" class="results">
			...
			<div class="status">
				<p>
					LIGHT up to {{ roundedTo(calcMax(curEndurance)*.299, 1) }}
				</p>
				<p>
					MEDIUM from {{ roundedTo(calcMax(curEndurance)*.3, 1) }} to {{ roundedTo(calcMax(curEndurance)*.699, 1) }}
				</p>
				<p>
					HEAVY from {{ roundedTo(calcMax(curEndurance)*.7, 1) }} to {{ roundedTo(calcMax(curEndurance)*.999, 1) }}
				</p>
			</div>
			<br/>
